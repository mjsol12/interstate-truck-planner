...
const ROWS = [
  ...
  { key: 'on_duty', label: 'On Duty (Not Driving)', color: tokens.colors.warning },
] as const
...
export default function EldLogSheet({ sheet }: EldLogSheetProps) {
  ...
  useEffect(() => {
    ...
    ctx.fillText("U.S. DEPARTMENT OF TRANSPORTATION — DRIVER'S DAILY LOG", 20, 28)
    ctx.font = '12px Inter, system-ui, sans-serif'
    ctx.fillText(`Day ${sheet.day_number} — Date: ${sheet.date_display}`, 20, 50)
    ...
    >
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', overflowX: 'auto' }}>
        <canvas
          ...
          style={{ border: `1px solid ${tokens.colors.border}`, borderRadius: 8 }}
        />
      </Box>
    </Box>
  )
