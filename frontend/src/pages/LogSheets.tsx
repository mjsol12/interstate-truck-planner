...
export default function LogSheets() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const withLogs = allTrips.filter(
        (t) => t.status === 'completed' && (t.route_data?.log_sheets?.length ?? 0) > 0,
      )
  ...
  const allRows = useMemo(() => flattenLogSheets(tripsWithLogs), [tripsWithLogs])
  ...
  const hasActiveFilters = filters.search.trim() !== '' || filters.tripId !== 'all'
    ...
    >
      ...
      {showDataPanel && (
        ...
        >
          ...
          >
            <Box sx={{ flex: { xs: '1 1 100%', sm: '1 1 280px' }, minWidth: 0, maxWidth: { sm: 360 } }}>
              <SearchInput
              ...
              >
                ...
                {tripsWithLogs.map((trip) => (
                  <MenuItem key={trip.id} value={trip.id}>
                    #{trip.id} — {trip.pickup_location} → {trip.dropoff_location}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
