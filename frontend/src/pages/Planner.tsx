    ...
    <Box sx={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
      ...
      <Grid container spacing={2} sx={{ alignItems: 'flex-start' }}>
        <Grid size={{ xs: 12, lg: 5 }}>
          <Panel title="Trip configuration" description="Required fields for route and HOS calculation">
            <Box
              component="form"
            ...
            >
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <TextField
                  ...
                  disabled={loading}
                  slotProps={{
                    htmlInput: { min: 0, max: 70, step: 0.5, 'aria-describedby': 'cycle-helper' },
              ...
              {result && !error && (
                <Box sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  ...
                  {result.status === 'completed' && logSheetCount > 0 && (
                    <Button component={RouterLink} to="/logs" variant="outlined" fullWidth size="small">
                      View {logSheetCount} log sheet{logSheetCount !== 1 ? 's' : ''}
                    </Button>
                  )}
                </Box>
              )}
        ...
        <Grid size={{ xs: 12, lg: 7 }}>
          <Box sx={{ position: { lg: 'sticky' }, top: { lg: 88 } }}>
            <Panel
              ...
              flush
            >
              <MapView routeData={result?.route_data} loading={loading} embedded />
            </Panel>
          </Box>
        </Grid>
      </Grid>
